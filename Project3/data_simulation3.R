...
# Parameters
alpha <- 5         # Intercept
beta_values = c(1, 2, 3) # Small/large treatment effect
...
simulate_and_evaluate <- function(n_clusters, R_per_cluster, alpha, beta, gamma2, sigma2) {
  ...
  # Simulate cluster-level random effects
  cluster_effects <- rnorm(n_clusters, mean = 0, sd = sqrt(gamma2))
  
  # Linear predictor for each cluster (log-scale mean)
  eta_i <- alpha + beta * X + cluster_effects
  ...
  eta <- rep(eta_i, each = R_per_cluster)
  
  # Poisson mean (exponential of linear predictor)
  mu <- exp(eta)
  ...
  fit <- glmer(Y ~ treatment + (1 | cluster), family = poisson(link = "log"), data = data)
...
write.csv(final_results, "aggregated_results3-1.csv", row.names = FALSE)
...
param_grid <- read.csv("all_feasible_designs.csv")
...
# Save the aggregated results to CSV
write.csv(final_results, "aggregated_results3-2.csv", row.names = FALSE)
